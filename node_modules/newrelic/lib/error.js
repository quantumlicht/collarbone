 ...
 */
function createError(transaction, exception, customParameters) {
  ...
  if (customParameters) {
    var ignored = [];
    if (transaction) ignored = transaction.agent.config.ignored_params;
    Object.keys(customParameters).forEach(function cb_forEach(param) {
      if (ignored.indexOf(param) === -1) params[param] = customParameters[param];
    });
  }
  ...
  if (transaction && transaction.url) {
    ...
    if (transaction.agent.config.capture_params) {
      ...
      transaction.agent.config.ignored_params.forEach(function cb_forEach(k) {
...
ErrorTracer.prototype.add = function add(transaction, exception, customParameters) {
  ...
  if (this.errors.length < MAX_ERRORS) {
    var error = createError(transaction, exception, customParameters);
